...
from ..data_def.Match import GoalData
...
def get_goal(page: Page, event) -> GoalData:
    attr = get_goal_attributes(event)
    scorer_link = attr['goal']
    if scorer_link is None:
        return None
    
    scorer = scrape_player_data(page, scorer_link)
    pen = "penalty" in attr["pen"].lower() if attr["pen"] else False
    assist_link = attr['assist']
    ...
    return GoalData(scorer=scorer, assist=assist, penalty=pen)    